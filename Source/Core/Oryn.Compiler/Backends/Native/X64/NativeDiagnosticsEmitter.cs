...
internal sealed class NativeDiagnosticsEmitter
{
    public string Emit(CompilerManifest Manifest, IReadOnlyList<IrInstruction> Instructions, string CPath, string AssemblyPath)
    {
        ...
        if (Manifest.SourcePath.Contains("Stage8", StringComparison.OrdinalIgnoreCase) || Manifest.SourcePath.Contains("Stage9", StringComparison.OrdinalIgnoreCase))
        {
            Builder.AppendLine(Manifest.SourcePath.Contains("Stage9", StringComparison.OrdinalIgnoreCase)
                ? "[ OK ] [ CONTRACT ] Stage 9 inherited module API contract validation passed."
                : "[ OK ] [ CONTRACT ] Stage 8 module API contract validation passed.");
            ...
            if (Manifest.SourcePath.Contains("Stage9", StringComparison.OrdinalIgnoreCase))
            {
                Builder.AppendLine("[ OK ] [ COMPOSE  ] Stage 9 generated source accepted before backend/native compilation.");
            }
        }
