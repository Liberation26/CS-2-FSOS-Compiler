...
case "$REQUESTED_STAGE" in
    ...
    1|stage1|Stage1|STAGE1)
        printf '[FAIL] [ RUNQEMU  ] Stage 9 development mode is active; Stage1 is not run by this script. Use Stage2, Stage3, Stage4, Stage5, Stage6, Stage7, Stage8, or Stage9.\n'
        exit 1
        ;;
...
with out.open('w', encoding='utf-8') as handle:
    ...
    for item in resolved:
        source = str(item.get('nativeSource', '')).lstrip('\ufeff').strip()
...
PY_SOURCES
    ...
    while IFS= read -r NativeSource; do
        # Manifest source files generated by older 1.0.x archives may contain a UTF-8 BOM.
        # Trim it here as a compatibility guard before checking or compiling paths.
        NativeSource="${NativeSource#$'\xef\xbb\xbf'}"
        NativeSource="${NativeSource//$'\r'/}"
