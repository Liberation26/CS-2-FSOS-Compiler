...
Oryn 1.0.4 is the first end-user OS generation milestone.
...
For 1.0.4, the only user-selectable module is:
...
Stage 9 remains the internal compiler proof for generated kernel template composition. Oryn 1.0.4 uses that proof as the engine behind the user-facing generated OS workflow.
...
```text
Tests/Generator/1.0.4/
```

Run them with:

```bash
Tests/Generator/1.0.4/run.sh
```
