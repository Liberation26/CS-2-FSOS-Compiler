...
LOG_FILE="$OUTPUT_DIR/03-forbidden-namespace-fails.log"
COMPILER_DLL="$ROOT_DIR/Source/Core/Oryn.Compiler/bin/Debug/net8.0/Oryn.Compiler.dll"
SOURCE_FILE="$SCRIPT_DIR/Sources/ForbiddenNamespaceKernel.cs"
OBJECT_FILE="$OUTPUT_DIR/ForbiddenNamespaceKernel.stage4.o"
...
[ "$STATUS" -ne 0 ] || fail "Forbidden namespace unexpectedly compiled. See: $LOG_FILE"
[ ! -f "$OBJECT_FILE" ] || fail "Forbidden namespace unexpectedly wrote object file: $OBJECT_FILE"
grep -q "Stage 4 module boundary rejected namespace 'Oryn.Kernel.UnsafeNative'" "$LOG_FILE" || fail "Expected forbidden-namespace diagnostic missing from: $LOG_FILE"

info "Forbidden Stage 4 namespace failed as expected"
